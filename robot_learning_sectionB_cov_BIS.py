...
class Agent:
    ...
    def calculate_policy_with_random_shooting(self, num_action_sequences, num_actions_per_sequence, environment):
        ...
        #cov=max_action*np.eye(2*num_actions_per_sequence)
        ...
        for time in range(10): #refit P n times
            ...
            for nb_seq in range(num_action_sequences):
                ...
                #print(cov[num_actions_per_sequence:,num_actions_per_sequence:])


                #seq_actions_x=np.random.multivariate_normal(means_x, cov[:num_actions_per_sequence,:num_actions_per_sequence])
                #seq_actions_y=np.random.multivariate_normal(means_y, cov[num_actions_per_sequence:,num_actions_per_sequence:])
            ...
            #cov = np.cov(np.concatenate((values_x,values_y),axis=0))
...
class MainProgram(arcade.Window):
    ...
    def on_draw(self):
        ...
        for i in range(1,len(sequences)-1):
            ...
            arcade.draw_line_strip(point_list=positions,color=[220,220,220],line_width=3)
